...
dir.create(file.path(here("data", "final", paste0(data_resolution, "_", clim))), 
           showWarnings = FALSE)
runs_dir <- here("data", "final", paste0(data_resolution, "_", clim))
# gap <- 0.1
# 
# flip_priority <- FALSE
...
nms <- gsub(".tif$", "", fls) %>% 
  gsub(here("data/final", paste0(data_resolution, "_", clim),"solution_run-"), "", .)
...
data.frame(t(data.frame(perc_tot = round(perc_tot, 2),
  ...
  write.csv(here("data", "final", paste0(data_resolution, "_", clim), "Table1.csv"))
...
count_sum_t %>% write.csv(here("data", "final", paste0(data_resolution, "_", clim), "country_summaries.csv"))

n_cell_count <- count_df %>% group_by(NAME_0) %>% summarise(n=n())

round(t(t(count_sum_t) / n_cell_count$n * 100), 2) %>%
  write.csv(here("data", "final", paste0(data_resolution, "_", clim), "country_summaries_perc.csv"))
...
count_sum_2 %>% 
  write.csv(here("data", "final", paste0(data_resolution, "_", clim), "Table2_detail.csv"))
...
c_sum_tab %>% 
  write.csv(here("data", "final", paste0(data_resolution, "_", clim), "Table2.csv"), row.names = FALSE)
